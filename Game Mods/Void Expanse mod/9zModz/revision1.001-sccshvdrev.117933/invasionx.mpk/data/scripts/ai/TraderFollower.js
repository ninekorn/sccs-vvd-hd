...
/*
 ...
 Following player's ship (even through jumpgates), doesn't lock on any targets.
 If in proximity of target's base, then dock and then disappear
 =====================================================================
 */
/* args:
 scope_ships:
...
{
    //parameters
    var follow_ship_id = npc.GetTag(SHIP_ID, "target");

    //--------------------------------------------
    // F O L L O W I N G   O W N E R

    // Finish quest
    if (ship.IsDocked(SHIP_ID))
    {
        //remove this ship
        actions.InvokeTrigger("onTraderArrived", { ship_id: SHIP_ID });

        generator.RemoveShip(SHIP_ID);
    }
    else
    {
        var targetBase = npc.GetTag(SHIP_ID, "target_base");
        if (ship.GetDistanceToObj(SHIP_ID, targetBase) < 40)
        {
            npc.Unstick(SHIP_ID);

            if (npc.IsAtBaseEntry(SHIP_ID, targetBase))
            {
                npc.StopFollowingRoute(SHIP_ID);
                npc.EnterBase(SHIP_ID, targetBase);
                npc.SetDecisionsPerSecond(SHIP_ID, 1);
            }
            else
            {
                npc.FollowRouteToBase(SHIP_ID, targetBase);
            }
        }
        else
        {
            if (!ship.IsCanControl(follow_ship_id)
                || !ship.IsCanControl(SHIP_ID))
            {
                npc.Unstick(SHIP_ID);
                //console.Print("This ship or another ship is not controllable");
            }
            else
            {
                npc.StickToObject(SHIP_ID, follow_ship_id);
                //console.Print("Sticking to follow ship");
            }

            // Travel through jumpgate if follow ship is going through it
            var jag = ship.EnteringJumpgate(follow_ship_id);
...
{
    // storage.Remove("_system_mercs", "merc_" + SHIP_ID); // no need - it deleted by Mercenary.js in its OnDied
    actions.InvokeTrigger("onTraderKilled", { ship_id: SHIP_ID }); //quest failed
}*/
