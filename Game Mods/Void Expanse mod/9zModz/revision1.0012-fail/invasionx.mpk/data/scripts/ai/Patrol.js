...
function OnAIInited()
{
    ...
    {
        // half of the ships will fly away on low HP
...
function Decision(args)
{
    ...
    // if no objectives, fill the array
    if (npc.CountObjectives(SHIP_ID) == 0)
    {
        // travel through all the jumpgates of the system
        var jumpgates = game.GetSystemJumpgates(sys_id);
        var nextJumpgateId = utils.SelectRandom(jumpgates);
        npc.AddObjective(SHIP_ID, "go_to_jumpgate", { jumpgate: nextJumpgateId });

        // travel through all the bases of the system
        var bases = game.GetSystemBases(sys_id);
        var nextBaseId = utils.SelectRandom(bases);

        npc.AddObjective(SHIP_ID, "go_to_base", { base_id: nextBaseId });
        npc.AddObjective(SHIP_ID, "stay_at_base", { time: 20 });

        if (bases.length > 1)
        {
            // go to some other base also
            var otherBaseId;
            do
            {
                otherBaseId = utils.SelectRandom(bases);
            }
            while (otherBaseId == nextBaseId);

            npc.AddObjective(SHIP_ID, "go_to_base", { base_id: otherBaseId });
            npc.AddObjective(SHIP_ID, "stay_at_base", { time: 20 });
        }
    }
    ...
    if (!npc.IsLocked(SHIP_ID))
    {
        ...
        for (var i = 0; i < args.scope_ships.length; i++)
        {
                ...
                && otherShip.disposition < 0)
            {
                ...
                isLocked = true;
                NpcLib.LockPatrolAndTurretsOnTarget(SHIP_ID, otherShip.id, getawaydist);
                break;
            }
        }
    }
    ...
    if (!isLocked
        && npc.CountObjectives(SHIP_ID) > 0)
    {
        ProcessObjectives(args);
    }
}

function ProcessObjectives(args)
{
    var currentObjective = npc.GetCurrentObjective(SHIP_ID);
    ...
    switch (currentObjective.name)
    {
        case "go_to_jumpgate":
        {
            npc.FollowRouteToObject(SHIP_ID, currentObjective.jumpgate);

            var dist = npc.GetDistanceToObj(SHIP_ID, currentObjective.jumpgate);
            if (dist < 14)
            {
                npc.NextObjective(SHIP_ID);
                //console.Print(SHIP_ID + " reached jumpgate " + currentObjective.jumpgate + " on a dist " + dist);
                npc.StopFollowingRoute(SHIP_ID);
                //// not enter jumpgate - just continue patrol in the system (it will restart the cycle)
                //npc.EnterJumpgate(SHIP_ID, currentObjective.jumpgate);
                npc.SetDecisionsPerSecond(SHIP_ID, 2);
            }
            break;
        }
        ...
        case "go_to_base":
            if (npc.IsAtBaseEntry(SHIP_ID, currentObjective.base_id))
            {
                npc.NextObjective(SHIP_ID);
                npc.StopFollowingRoute(SHIP_ID);
                npc.EnterBase(SHIP_ID, currentObjective.base_id);
                npc.SetDecisionsPerSecond(SHIP_ID, 1);
            }
            else
            {
                npc.FollowRouteToBase(SHIP_ID, currentObjective.base_id);
            }
            break;

        case "stay_at_base":
            // console.Print("i'm on base, left " + (currentObjective.time - npc.GetDockedTime(SHIP_ID)));
            if (npc.GetDockedTime(SHIP_ID) > currentObjective.time
				&& !ship.IsInDialog(SHIP_ID))
            {
                npc.NextObjective(SHIP_ID);
