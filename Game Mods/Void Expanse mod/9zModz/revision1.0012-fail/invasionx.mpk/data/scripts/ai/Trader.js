/*
 ...
 Desc:
 Flies from one station to another, and then disappears through jumpgate.
 ====================================================================
 */
...
function OnAIInited()
{
    ...
    npc.SetBehavior(SHIP_ID, "run_away_on_hp", 100.0); //do not engage in battles
}
...
function Decision(args)
{
    ...
    if (npc.CountObjectives(SHIP_ID) == 0)
    {
        ...
        else
        {
            ...
            if (bases.length > 0)
            {
                npc.AddObjective(SHIP_ID, "go_to_base", { base_id: utils.SelectRandom(bases) });
                npc.AddObjective(SHIP_ID, "stay_at_base", { time: 20 });
            }
        }
        ...
        npc.AddObjective(SHIP_ID, "go_to_jumpgate", { jumpgate: pickJumpgate });
    }
    ...
    //if !locked process objectives (else battle is top priority)
...
//--------------------------------------------------
// separate function, responsible for processing objectives
//
//--------------------------------------------------
function ProcessObjectives(args)
{
    ...
    var sys_id = npc.GetCurrentSystemID(SHIP_ID);
    /*
     object: name = name of objective,
     param1 = param1
     */

    // go_to_base objective
    if (currentObjective.name == "go_to_base")
    {
    ...
    // stay_at_base objective
    else if (currentObjective.name == "stay_at_base")
    {
    ...
    //go_to_jummpgate objectvie
    else if (currentObjective.name == "go_to_jumpgate")
    {
