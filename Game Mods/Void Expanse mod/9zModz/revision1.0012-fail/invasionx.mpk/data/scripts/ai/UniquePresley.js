...
{
    var presley = storage.GetGlobal("presley.stuff." + SHIP_ID);

    if (presley.arrived == true)
    {
        if (presley.repair_step == 0)
        {
            // do nothing, wait
            npc.StickToObject(SHIP_ID, presley.target_jumpgate);
        }
        else if (presley.repair_step == 1)
        {
            // repair jumpgate, use special device
            npc.StickToObject(SHIP_ID, presley.target_jumpgate);
            npc.DeviceActivateOnObject(SHIP_ID, "device_mining_fake", presley.target_jumpgate);
        }
        else // if repair_step == 2
        {
            //repaired! do nothing again
            npc.StickToObject(SHIP_ID, presley.target_jumpgate);
        }
    }
    else
