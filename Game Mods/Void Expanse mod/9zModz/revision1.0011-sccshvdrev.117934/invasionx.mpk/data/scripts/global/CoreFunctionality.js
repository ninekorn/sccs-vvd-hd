    ...
    if (preset.name == "order")
    {
        ...
        if (orderCount <= 10)
        {
            for (var i = 0; i < 3; i++)
            {
                ...
                var id = generator.AddNPCShipToSystem($i0001, "FinalSystemSoldier", 5, picked, args.system_id, coords.x, coords.y, { class: "order.soldier", meta: "human", sex: "male", xmlnpcship: picked }); // Order soldier
        ...
        if (xenCount <= 10)
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                ...
                var id = generator.AddNPCShipToSystem($i0002, "FinalSystemSoldier", 5, picked, args.system_id, coord.x + 9, coord.y, { class: "alien.soldier", xmlnpcship: picked }); // Xengatarn
...
function ExodusSystemPopulation(args, inf, jag_ids, exodus)
{
    ...
    for (var i = civShips; i < 3; i++)
    {
        ...
        var id = generator.AddNPCShipToSystem($i0003, "ExodusCivilianVessel", 5, "special_human_tradership", args.system_id, coords.x, coords.y, { class: "freedom.civilian.ship", xmlnpcship: "special_human_tradership" }); // Civilian vessel
...
function OnGuardSpawn(args)
{
    ...
    for (var i = 0; i < guards_respawn.length; i++)
    {
        ...
        if (systems.indexOf(system) >= 0)
        {
            ...
            var id = generator.AddNPCShipToSystem($i0004, "TreasureGuard", inf.danger_level, random_type, system, coords.x + MathExt.RandInt() % 3, coords.y + MathExt.RandInt() % 3, { class: "pirate", crate: crate, xmlnpcship: random_type}); // Pirate
...
function RestorePiratesPopulation(args, inf, jag_ids)
{
    ...
    if (piratesCount < 30) //no less then 30 pirates in every system
    {
        ...
        var id = generator.AddNPCShipToSystem($i0006, "StickToPointAggr", inf.danger_level, random_type, args.system_id, coords.x, coords.y, { class: "pirate", stick_coords_x: stickCoords.x, stick_coords_y: stickCoords.y, xmlnpcship: random_type }); // Pirate
        relations.SetShipFaction(id, "pirates");
        ship.SetShipAsArrivedFromJumpgate(id, jag_id);
...
function RestoreAliensPopulation(args, inf, jag_ids)
{
    ...
    if (aliensCount < 30) //no less then 30 aliens in every alien system
    {
        ...
        var id = generator.AddNPCShipToSystem($i0007, "StickToPointAggr", inf.danger_level, random_type, args.system_id, coords.x, coords.y, { class: "alien", sex: "male", stick_coords_x: stickCoords.x, stick_coords_y: stickCoords.y, xmlnpcship: random_type}); // Xengatarn
        relations.SetShipFaction(id, "aliens");
        ship.SetShipAsArrivedFromJumpgate(id, jag_id);
...
function SpawnPirateHeadhunter(args, inf, jag_ids, max_number)
{
    ...
    var id = generator.AddNPCShipToSystem($i0008, "HeadHunter", inf.danger_level, random_type, args.system_id, coords.x, coords.y, { class: "pirate", target: playerShip, xmlnpcship: random_type }); // Pirate Headhunter
    relations.SetShipFaction(id, "pirates");
    ship.SetShipAsArrivedFromJumpgate(id, jag_id);
...
function SpawnAlienHeadhunterIfPossible(args, inf, jag_ids, max_number)
{
    ...
    var id = generator.AddNPCShipToSystem($i0009, "HeadHunter", inf.danger_level, random_type, args.system_id, coords.x, coords.y, { class: "pirate", target: playerShip, xmlnpcship: random_type }); // Xengatarn Seeker
    relations.SetShipFaction(id, "aliens");
    ship.SetShipAsArrivedFromJumpgate(id, jag_id);
