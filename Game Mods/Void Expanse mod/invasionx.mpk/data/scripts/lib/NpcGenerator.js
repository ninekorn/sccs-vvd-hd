...
var NpcGenerator = {
    ...
    SpawnNpcsPointsWithTreasure: function (args, numOfPoints)
    {
        ...
        for (var i = 0; i < numOfPoints; i++)
        {
                ...
                && !generator.AreThereAnyObjects(args.sys_id, coords.x, coords.y, 20))
            {
                ...
                var id = generator.AddNPCShipToSystem($i0002, "TreasureGuard", inf.danger_level, random_type, args.sys_id, coords.x + MathExt.RandInt() % 3, coords.y + MathExt.RandInt() % 3, { class: "pirate", crate: cont_id, xmlnpcship: random_type}); // Pirate
    ...
    SpawnPirateBlockposts: function (args, numOfPoints)
    {
        ...
        for (var i = 0; i < numOfPoints; i++)
        {
            ...
            var id = generator.AddNPCShipToSystem($i0003, "Turret", inf.danger_level + 10, "special_pirates_platform", system_id, rescoord.x, rescoord.y, { class: "pirate", xmlnpcship: "special_pirates_platform"}); // Pirate Den
            relations.SetShipFaction(id, "pirates");
            ...
            for (var j = 0; j < minionsCount; j++)
            {
                ...
                var id = generator.AddNPCShipToSystem($i0004, "PirateInWaiting", inf.danger_level + 10, "generic_pirates_fang_01", system_id, rescoord.x + 19 * Math.cos(ang), rescoord.y + 19 * Math.sin(ang), { class: "pirate", xmlnpcship: "generic_pirates_fang_01" }); // Pirate Guard
                relations.SetShipFaction(id, "pirates");
            }
        }
    },
    ...
    SpawnTurretsOfType: function (args, coords, quantity, level, turret_radius, ai_behavior, xml_ship, faction, tags, parsedAngle, base_xml, sys_id, base_id) {
        ...
        if (!isNaN(base_xml) || base_xml != null)
        {
        ...
        else // if (base_xml.substring(0, 7) == "special")
        {
            ...
            if (isNaN(base_xml) || base_xml == null)
            {
                console.PrintError("null base_xml");
            }
    ...
    /*
    SpawnTurretsOfType: function (args, coords, quantity, level, turret_radius, ai_behavior, xml_ship, faction, tags)
    {
        var angstep = Math.PI * 2 / quantity;
        var origoffset = MathExt.RandRangeDouble(0, Math.PI);
    ...
    SpawnNpcsStep: function (args, npcs_count, select_tags, faction, name, behaviors, tags)
    {
        ...
        for (; step < npcs_count;)
        {
            ...
            for (var groupMemberIndex = 0; groupMemberIndex < groupsize; groupMemberIndex++)
            {
                ...
                //var randomXML = Math.floor(Math.random() * ((arrayOfPiratesXML.length) - 0) + 0);
                tags.xmlnpcship = shipByRandom.xml_type;//arrayOfPiratesXML[randomXML];//"generic_pirates_shuttle_01";//shipByRandom.xml_type;
