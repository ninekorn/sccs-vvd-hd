...
function OnCheckRequirements() {
    ...
    if (npc.GetTag(npid, "class") == "marketSELLRcs") {
        return true;
    }
    return false;
}
...
function OnDialogue(args) {
    ...
    if (pin == NO_INPUT) {
        ...
        schid = items.GetShipStationStorageContainerId(pshp, css);

        var iicg = items.GetItemsAndCargo(schid);
        lorcivl = [];
        var i;
    ...
    ////////////////////////////BOOSTER/////////////////////////////////////////////
        ...
        var ii;
        for (ii = 0; ii < lorcivl.length; ii++) {

            var pmd = CalculateSellPriceOfItemModifier(pshp);
            var obj = generator.GetItemByXmlID(lorcivl[ii].xml_id);
            var spce = Math.ceil(obj.price * pmd);

            var cstg = lorcivl[ii].xml_id.replace(/_/g, " ");
    ...
    else if (pin >= 1300000 && pin < 1300000 + tnorcivl) {
    ...
    else if (pin >= 13000000 && pin < 13000000 + lorcivl.length) {
