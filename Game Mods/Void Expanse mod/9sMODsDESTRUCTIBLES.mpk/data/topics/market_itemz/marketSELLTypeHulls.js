...
function OnCheckRequirements() {
    ...
    if (npc.GetTag(npid, "class") == "marketSELLHulls") {
        return true;
    }
    return false;
}
...
function OnDialogue(args) {
    ...
    if (pin == NO_INPUT) {
        ...
        for (i = 0; i < iccg.length; i++) {
            if (iccg[i].xml_id.slice(0, 4) == "hull") {
                lohuivl.push(iccg[i]);
            }
        }

        topic.AddChoice(130000, "Hulls");
    ...
    ////////////////////////////BOOSTER/////////////////////////////////////////////
        ...
        var ii;
        for (ii = 0; ii < lohuivl.length; ii++) {

            var pmd = CalculateSellPriceOfItemModifier(pshp);
            var obj = generator.GetItemByXmlID(lohuivl[ii].xml_id);
            var shpce = Math.ceil(obj.price * pmd);

            var cstg = lohuivl[ii].xml_id.replace(/_/g, " ");
            topic.AddChoice(1300000 + ii, cstg.toUpperCase() + " " + "Price: " + shpce + "$");
        }
    ...
    else if (pin >= 1300000 && pin < 1300000 + tnohuivl) {
    ...
    else if (pin >= 13000000 && pin < 13000000 + lohuivl.length) {
