...
var droneAIRepair = "SC_Drone_Repair_AI_";
...
function OnCheckRequirements()
{
    ...
    if (npc.GetTag(npc_id, "class") == "droneManagerRepair")
    {
        return true;
    }
    return false;
...
function OnDialogue(args)
{
    ...
    if (state == 0)
    {
        if (!isPlayerShipDocked)
        {
            ...
            {       
                totalDronesQuant = 0;
                droneAIRepair = "SC_Drone_Repair_AI_";
                //selectedDroneAI = droneAIRepair;
                selectedDroneType = "repair";
                selectedDroneXML = "";
            ...
            else if (input == 1)
            {
                ...
                {
                    if (itemsAndCargo[i].xml_id.substring(0, 12) == "drone_repair")
                    {
            ...
            {		
				...
				for (var i = 0; i < arrayOfPoserer.length; i++)
				{
					if (arrayOfPoserer[i].id != -1)
					{
						...
						{
							if (player_id == npc.GetTag(arrayOfPoserer[i].id, "ownerPlayerShipId") &&  npc.GetTag(arrayOfPoserer[i].id, "typeOfDrone") == "repair")
							{								
            ...
            else if (input == 13) 
            {
                ...
                {					
                    for (var r = 0; r < maxQuantity; r++)
                    {
                        ...
                        SC_AI_Drone_Manager_Utilities.setFinalGlobalDroneArrayRepair(id1, index, weaponDistanceType, weaponPropulsionType);
