...
function OnCheckRequirements()
{
    ...
    if (npc.GetTag(npc_id, "class") == "droneManagerCombat")
    {
        return true;
    }
    return false;
...
function OnDialogue(args)
{
    ...
    if (state == 0)
    {
        if (!isPlayerShipDocked)
        {
            ...
            {       
                totalDronesQuant = 0;
                droneAICombat = "SC_Drone_Combat_AI_";
                //selectedDroneAI = droneAICombat;
                selectedDroneType = "combat";
                selectedDroneXML = "";
            ...
            {		
				...
				for (var i = 0; i < arrayOfPoserer.length; i++)
				{
					if (arrayOfPoserer[i].id != -1)
					{
						...
						{
							if (player_id == npc.GetTag(arrayOfPoserer[i].id, "ownerPlayerShipId") &&  npc.GetTag(arrayOfPoserer[i].id, "typeOfDrone") == "combat") //&& selectedDroneXML.indexOf("ballistic")
							{								
                                ...
                                var currentCom = storage.GetGlobal("currentCommand" + id1);

                                currentCom.command = 9;

                                storage.SetGlobal("currentCommand_switch_" + arrayOfPoserer[i].id, 1);
                                storage.SetGlobal("currentCommand" + arrayOfPoserer[i].id, currentCom);
								//storage.SetGlobal("currentCommand_switch_" + id1, 1);
								//storage.SetGlobal("currentCommand" + id1, { id: id1, command: 9, formation: chosenPosition, lastCommand: 0, wepPropType: weaponPropulsionType, wepDistType: weaponDistanceType, droneIndex: index, addFriend: 0, baseVicinity: -1 }); //, behavior: behavior
            ...
            else if (input >= 700 && input <= 700 + droneCounterer)
            {
				...
				storage.SetGlobal("currentCommand" + currentSelectedDrone, { id: currentSelectedDrone, command: 1, formation: forma, lastCommand: -1, wepPropType: wepProp, wepDistType: wepDist, droneIndex: droneIndex, addFriend: 0, baseVicinity: -1 }); //, behavior: behavior
			}
            ...
            else if (input == 1000 || input == 1001 || input == 1002 || input == 1003 || input == 1004 || input == 1005)
			{
                ...
                if (maxQuantity > 0)
                {
                    for (var r = 0; r < maxQuantity; r++)
                    {
                        ...
                        /*var id1 = generator.AddNPCShipToSystem(selectedDroneType, selectedDroneAI, level, selectedDroneXML, sys_id, coords.x + (5 * Math.cos(randomPos * Math.PI / 180)), coords.y + (5 * Math.cos(randomPos * Math.PI / 180)), { ownerPlayerShipId: player_id, class: "drone", someClass: "drone", droneIndex: index, wepPropType: weaponPropulsionType, wepDistType: weaponDistanceType, typeOfDrone: "combat", aDock: 1, sDock: 0, aDef: 0, mDef: 0, sDefLoc: 0, aRep: 0, mRep: 0, minHP: 0.15, formation: chosenPosition, xml: tempDroneXML });
