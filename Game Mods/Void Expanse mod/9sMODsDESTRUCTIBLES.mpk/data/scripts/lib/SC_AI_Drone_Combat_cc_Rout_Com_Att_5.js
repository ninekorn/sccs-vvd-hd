...
include(SC_AI_Drone_Combat_cc_Rout_Com_Evad_5.js);
include(SC_AI_Drone_Combat_cc_Rout_Com_Att_Tec0_5.js);
include(SC_AI_Drone_Combat_cc_Rout_Com_Att_Tec1_5.js);
include(SC_AI_Drone_Combat_cc_Rout_Com_Att_Tec2_5.js);
include(SC_AI_Drone_Combat_cc_Rout_Com_Att_Tec3_5.js);

var lockdist = 20;
var randomPos = -1;
var distPlayerToEnemy = 0;

var SC_AI_Drone_Combat_cc_Rout_Com_Att_5 =
{
    npcCombatRoutine: function (ceData)
    {
        ...
        if (generator.ShipExists(ceData.ts) && ship.GetCurrentValue(ceData.ts, "structure") > 0)
        {
            ...
            if (distPlayerToEnemy <= 25) // && distPlayerToDrone <= 20
            {
                ...
                ceData = SC_AI_Drone_Combat_cc_Rout_Com_Evad_5.npcIsShouldEvade(ceData, otherShipCoord);
                ...
                if (ceData.evad == 0) //ceData.evad != 1
                {
                    ...
                    else if (ceData.mReset == 1)
                    {
                        ...
                        if (ceData.hasNA == 1)
                        {
                            if (ceData.tec == 0)
                            {
                                ceData = SC_AI_Drone_Combat_cc_Rout_Com_Att_Tec0_5.npcAttackTec0(ceData, otherShipCoord, distDroneToEnemy);
                            }
                            else if (ceData.tec == 1) {
                                ceData = SC_AI_Drone_Combat_cc_Rout_Com_Att_Tec1_5.npcAttackTec1(ceData, otherShipCoord, distDroneToEnemy);
                            }
                            else if (ceData.tec == 2) {
                                ceData = SC_AI_Drone_Combat_cc_Rout_Com_Att_Tec2_5.npcAttackTec2(ceData, otherShipCoord, distDroneToEnemy);
                            }
                            else if (ceData.tec == 3) {
                                ceData = SC_AI_Drone_Combat_cc_Rout_Com_Att_Tec3_5.npcAttackTec3(ceData, otherShipCoord, distDroneToEnemy);
                            }
                        }
                    }
                }
