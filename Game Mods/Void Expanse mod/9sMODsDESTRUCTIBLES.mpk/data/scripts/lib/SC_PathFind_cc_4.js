...
include(SC_PathFindCheckAllSides_cc_4.js);
include(SC_Utilities.js);
include(SC_Pathfind_Heap_cc_4.js);
    ...
    npcPathFind: function (gridWorldSize, nodeRadius, cData, forceUseRetracePathSwtc, pathfindCounterSwtch) //
    {
        ...
        if (cData.dog.openSet.length > 0) {
            if (cData.dog.openSet.length > maxOpensetLengthWhenThePathfindLoops*FramePathfindIteratedCounterMax) // the pathfind openSet of new tiles is starting to get bigger and bigger and the target tile isn't being found. stop the pathfind for now, to fix later.
            {
                ...
                cData.dog.openSet = SC_Pathfind_Heap_cc_4.heapSort(cData.dog.openSet, cData.dog.openSet.length - 1, "fcost"); //original hcost
            ...
            else //if (cData.dog.openSet.length > maxOpensetLengthWhenThePathfindLoops*FramePathfindIteratedCounterMax)
            {
                ...
                if (forceUseRetracePathSwtc != 1) {
                    //------------------------------------
                    //------------------------------------
                    var someData = SC_PathFindCheckAllSides_cc_4.checkAllSidesGridIndex(node, gridWorldSize, cData, cData.lip);
                    //------------------------------------
                    //------------------------------------
                        ...
                        if (someData.extraTiles.length > 0) {
                            ...
                            for (var i = 0; i < someExtratiles.length; i++) {
                                ...
                                if (cData.log[gridIndex] == null) {
                                    ...
                                    var pathData = SC_PathFindInitGrid_Data_cc_4_merge.npcGetGridDataMerged(gridWorldSize, nodeRadius, gridIndex, cData, node,0);

                                    //var pathData = SC_PathFindInitGrid_Data_cc_4_merge.npcGetGridDataMerged(gridWorldSize, nodeRadius, gridIndex, cData, node);
                    ...
                    if (node.isStationTile == 1 || node.isStationTile == 2) // currently unassigned
                    {
                        if (node.isStationTile == 2)
                        {
                            //------------------------------------
                            //------------------------------------
                            var someNewStationTilesUnwalkableNeighboorsData = SC_PathFindCheckAllSides_cc_4.checkAllSidesGridIndex(node, gridWorldSize, cData, cData.lip);
                            //------------------------------------
                            //------------------------------------
                                ...
                                if (someNewStationTilesUnwalkableNeighboorsData.extraTiles.length > 0) {
                                    ...
                                    for (var i = 0; i < someExtratiles.length; i++) {
                                        ...
                                        if (cData.log[gridIndex] == null) {
                                            ...
                                            var pathData = SC_PathFindInitGrid_Data_cc_4_merge.npcGetGridDataMerged(gridWorldSize, nodeRadius, gridIndex, cData, node, 1 );
                    ...
                    if (someData.neighboors.length > 0) {
                        ...
                        for (var j = 0; j < someData.neighboors.length; j++) {
                            if (someData.neighboors[j].node != null) {
                                ...
                                var gcost = node.gcost + SC_PathFindUtilities_cc_4.npcCheckDistance(node.worldPosition, someData.neighboors[j].node.worldPosition);
                                ...
                                if (gcost < someData.neighboors[j].node.gcost || cData.log[someData.neighboors[j].node.gridIndex].grid[someData.neighboors[j].node.index].open == 0) {
                                    ...
                                    someData.neighboors[j].node.hcost = SC_PathFindUtilities_cc_4.npcCheckDistance(someData.neighboors[j].node.worldPosition, cData.ltp);
