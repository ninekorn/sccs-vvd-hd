...
include(SC_AI_PathFind_Mining_NodeDistance_2.js);
include(SC_AI_PathFind_Mining_CheckAllSides_2.js);
include(SC_Utilities.js);
include(SC_AI_PathFind_Mining_Heap_2.js);
include(SC_AI_PathFind_Mining_InitGrid_2.js);
...
var SC_AI_PathFind_Mining_2 =
{
    npcPathFind: function (gridWorldSize, nodeRadius, cData) //
    {
        ...
        if (cData.dog.openSet.length > 0)
		{
            ...
            cData.dog.openSet = SC_AI_PathFind_Mining_Heap_2.heapSort(cData.dog.openSet, cData.dog.openSet.length - 1);
            ...
            //------------------------------------
            //------------------------------------
            var someData = SC_AI_PathFind_Mining_CheckAllSides_2.checkAllSides(node, gridWorldSize, cData, cData.lip);
            //------------------------------------
            //------------------------------------
                ...
                if (someData.extraTiles.length > 0) 
				{
                    ...
                    for (var i = 0; i < someExtratiles.length; i++)
                    {
                        ...
                        if (cData.log[gridIndex] == null) {
                            ...
                            //var pathData = SC_AI_PathFind_Mining_InitGrid_Data_2.npcGetGridData(gridWorldSize, nodeRadius, gridIndex, cData, node);
                            //var pathData = SC_AI_PathFind_Mining_InitGrid_2.npcGetGridData(gridWorldSize, nodeRadius, gridIndex, cData, node);
                            var pathData = SC_AI_PathFind_Mining_InitGrid_2.npcBuildGrid(gridWorldSize, nodeRadius, gridIndex, cData, node);
            ...
            if (someData.neighboors.length > 0) {
                ...
                for (var j = 0; j < someData.neighboors.length; j++)
                {
                    if (someData.neighboors[j].node != null)
                    {
                        ...
                        var gcost = node.gcost + SC_AI_PathFind_Mining_NodeDistance_2.checkNodeDistance(node.worldPosition, someData.neighboors[j].node.worldPosition);
                        ...
                        if (gcost < someData.neighboors[j].node.gcost || cData.log[someData.neighboors[j].node.gridIndex].grid[someData.neighboors[j].node.index].open == 0) 
                        {
                            ...
                            someData.neighboors[j].node.hcost = SC_AI_PathFind_Mining_NodeDistance_2.checkNodeDistance(someData.neighboors[j].node.worldPosition, cData.ltp);
