...
include(SC_AI_Drone_Combat_cc_Rout_For_SFWP_5.js);
include(SC_AI_Drone_Combat_cc_Rout_Com_FndTarg_5.js);
include(SC_AI_Drone_Combat_cc_Rout_Com_Att_5.js);
...
var SC_AI_Drone_Combat_cc_Rout_Com_5 =
{
    AICombatRoutine: function (cData, ceData) {
        ...
        if (cData.lLSwtcCounter >= 0) // 5 frames before checking for targets or unwalkable tiles. //19*10 was a bit not enough
        {
            if (ceData.lLSwtc == 0) // main check for find target. if no target, this script will not be used.
            {
                ...
                cData.cdtSwtc = 1;
                cData.cdSwtc = 1;
                tempLceData = SC_AI_Drone_Combat_cc_Rout_Com_FndTarg_5.npcFindTarget(ceData);
                ceData = tempLceData;
                //cData.lLSwtc = 1;
                //ceData.lLSwtc = 1;
            }
            ...
            if (!generator.ShipExists(ceData.ts)) {
                ...
                if (ship.GetCurrentValue(ceData.ts, "structure") > 0)
                {
                    ...
                    if (npc.IsLocked(ceData.objt.nid)) {
                        ...
                        //TO READD LATER
                        //TO READD LATER
                        tempLceData = SC_AI_Drone_Combat_cc_Rout_Com_Att_5.npcCombatRoutine(ceData);
                        ceData = tempLceData;
                        //TO READD LATER
                        //TO READD LATER
                        //TO READD LATER
                        ...
                        if (generator.ShipExists(ceData.ts) && ship.GetCurrentValue(ceData.ts, "structure") > 0) {
                            ...
                            if (distPlayerToEnemy <= lockDist) // && distPlayerToDrone <= 20
                            {
                                ...
                                if (distDroneToEnemy <= lockDist) {
                                    ...
                                    cData = SC_AI_Drone_Combat_cc_Rout_For_SFWPP_5.npcPathfindToWaypoint(cData);
                                    ...
                                    if (cData.objt.command == 0 && ceData.ts == -1) {
                                        cData.mSwtc = 1;
                                        cData = SC_AI_Drone_Combat_cc_Rout_For_SFWP_5.npcStayInFormation(cData);
                                    }
...
var SC_AI_Drone_Combat_cc_Rout_Com_5 =
{
    ...
    {       
        ...
        //{
        //    cData = SC_AI_Drone_Combat_cc_Rout_For_SFWPP_5.npcStayInFormationPathfind(cData);
        //}
       ...
       //         {
        ...
        //    cData.mSwtc = 1;
        //    cData.fSwtc = 1;
        //    ceData = SC_AI_Drone_Combat_cc_Rout_Com_Att_5.npcCombatRoutine(ceData);
        //}
        ...
        //    if (cData.mSwtc != 3 && cData.mSwtc != 4 && cData.mSwtc != 5 && cData.mSwtc != 6 && cData.mSwtc != 7) {
         ...
         //       //cData.sSwtc = 2;
         //       cData = SC_AI_Drone_Combat_cc_Rout_For_SFWPP_5.npcPathfindToWaypoint(cData, cData.objt.bid);
          //  }
          //  else {
//
