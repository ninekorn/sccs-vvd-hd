...
include(SC_AI_Drone_Mining_Rout_For_FWP_1.js);
include(SC_AI_Drone_Mining_Rout_Com_1.js);
include(SC_AI_Drone_Mining_Reset_Speed_1.js);
include(SC_AI_Drone_Mining_Friendlies_1.js);
...
var SC_AI_Drone_Mining_Rout_For_1 =
{
    AIRoutineInit: function (currentObjective,addFriendSwitch) 
	{
		arrayOfFriendlies = SC_AI_Drone_Mining_Friendlies_1.AIAddFriendlies(currentObjective,addFriendSwitch);
        ...
        if (cData.length >= currentObjective.droneIndex && ceData.length >= currentObjective.droneIndex && cData[currentObjective.droneIndex] != null && ceData[currentObjective.droneIndex] != null )
        {
            ...
            if (currentObjective.speedSwitch == 1)
            {
                if (nData != null && pData != null)
                {
                    ...
                    currentFormationWaypoint = SC_AI_Drone_Mining_Rout_For_FWP_1.npcGWFP(currentObjective, 5);
                    ...
                    if (distToWaypoint >= cData.DistanceToAsteroid) 
					{
                        ...
                        dataToReturn = SC_AI_Drone_Mining_Rout_Com_1.AICombatRoutine(cData[currentObjective.droneIndex], ceData[currentObjective.droneIndex]);
            ...
            {
                SC_AI_Drone_Mining_Reset_Speed_1.AICombatResetSpeed(currentObjective);
            }
        }
        else
        {
			if(cData.length < currentObjective.droneIndex || cData[0] == null)
			{
				cDat =
				{
					...
					cFWP: { x: null, y: null }, //waypoint formation
                    cLFWP: { x: null, y: null }, //for waypoint
					cFWPD: 0,
					...
					dog: [], //dog stands for data of grid
					log: [], //log stands for 
					pfc: -2,
					cdSwtc: 0, //
					cdtSwtc: 0,
					loRLDOT: 0, //left or right dot
					stopS: 1,
					stopSC: 0,
					stopSCM: 5,
					noFWP: 0,
			...
			else if(cData[currentObjective.droneIndex] == null)
			{
				cDat =
				{
					...
					cFWP: { x: null, y: null },
					cLFWP: { x: null, y: null },
					cFWPD: 0, // current formation waypoint distance
