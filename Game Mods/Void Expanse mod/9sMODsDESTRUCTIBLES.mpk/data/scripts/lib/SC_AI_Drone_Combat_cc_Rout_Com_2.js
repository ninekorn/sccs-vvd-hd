...
include(SC_AI_Drone_Combat_cc_Rout_For_SFWP_2.js);
include(SC_AI_Drone_Combat_cc_Rout_Com_FndTarg_2.js);
include(SC_AI_Drone_Combat_cc_Rout_Com_Att_2.js);
...
var SC_AI_Drone_Combat_cc_Rout_Com_2 =
{
    AICombatRoutine: function (cData, ceData) {
        ...
        if (cData.lLSwtcCounter >= 0) // 5 frames before checking for targets or unwalkable tiles. //19*10 was a bit not enough
        {
            if (ceData.lLSwtc == 0) // main check for find target. if no target, this script will not be used.
            {
                ...
                cData.cdtSwtc = 1;
                cData.cdSwtc = 1;
                tempLceData = SC_AI_Drone_Combat_cc_Rout_Com_FndTarg_2.npcFindTarget(ceData);
                ceData = tempLceData;
                //cData.lLSwtc = 1;
                //ceData.lLSwtc = 1;
            }
            ...
            if (!generator.ShipExists(ceData.ts)) {
                ...
                if (ship.GetCurrentValue(ceData.ts, "structure") > 0)
                {
                    ...
                    if (npc.IsLocked(ceData.objt.nid)) {
                        ...
                        //TO READD LATER
                        //TO READD LATER
                        tempLceData = SC_AI_Drone_Combat_cc_Rout_Com_Att_2.npcCombatRoutine(ceData);
                        ceData = tempLceData;
                        //TO READD LATER
                        //TO READD LATER
                        //TO READD LATER
                        ...
                        if (generator.ShipExists(ceData.ts) && ship.GetCurrentValue(ceData.ts, "structure") > 0) {
                            ...
                            if (distPlayerToEnemy <= lockDist) // && distPlayerToDrone <= 20
                            {
                                ...
                                if (distDroneToEnemy <= lockDist) {
                                    ...
                                    cData = SC_AI_Drone_Combat_cc_Rout_For_SFWPP_2.npcPathfindToWaypoint(cData);
                                    ...
                                    if (cData.objt.command == 0 && ceData.ts == -1) {
                                        cData.mSwtc = 1;
                                        cData = SC_AI_Drone_Combat_cc_Rout_For_SFWP_2.npcStayInFormation(cData);
                                    }
...
var SC_AI_Drone_Combat_cc_Rout_Com_2 =
{
    ...
    {       
        ...
        //{
        //    cData = SC_AI_Drone_Combat_cc_Rout_For_SFWPP_2.npcStayInFormationPathfind(cData);
        //}
       ...
       //         {
        ...
        //    cData.mSwtc = 1;
        //    cData.fSwtc = 1;
        //    ceData = SC_AI_Drone_Combat_cc_Rout_Com_Att_2.npcCombatRoutine(ceData);
        //}
        ...
        //    if (cData.mSwtc != 3 && cData.mSwtc != 4 && cData.mSwtc != 5 && cData.mSwtc != 6 && cData.mSwtc != 7) {
         ...
         //       //cData.sSwtc = 2;
         //       cData = SC_AI_Drone_Combat_cc_Rout_For_SFWPP_2.npcPathfindToWaypoint(cData, cData.objt.bid);
          //  }
          //  else {
//
