...
var SC_AI_Drone_Combat_rc_Rout_Com_Att_Tec2_5 =
{
    npcAttackTec2: function (ceData, otherShipCoord, distDroneToEnemy) {
        ...
        //console.PrintError(ceData.objt.wepPropType);

        if (ceData.objt.wepPropType != "missile")
        {
            if (ceData.tec2 >= 2) {
        ...
        else
        {
            ...
            if (isLeftOrRight > 0)//is RIGHT
            {
                ...
                randomPos = (Math.random() * (maxR - minR) + 0);
                pointVeloFront = {
            ...
            if (distDroneToEnemy > ceData.npcLD && distDroneToEnemy > range1 && alignedTowardsEnemy0 < 0.85) {
                ...
                npc.StickToPoint(ceData.objt.nid, newPointer.x + (ceData.nData.nForward.x * distDroneToEnemy * range1), newPointer.y + (ceData.nData.nForward.y * distDroneToEnemy * range1), 0);
            ...
            else if (distDroneToEnemy > range1)
            {
                modDist = distDroneToEnemy;
            ...
            else if (alignedTowardsEnemy0 < 0.99 && distDroneToEnemy <= range2)
            {
                if (ceData.tec2 == 0) {
                    if (!npc.IsLocked(ceData.objt.nid)) {
                        npc.LockOnTarget(ceData.objt.nid, ceData.ts, 0);
                ...
                if (enemyAlignedTowardsNPC >= 0.90 && ceData.stopS == 0 && distDroneToEnemy < 8)
                {
                    npc.GoBackward(ceData.objt.nid);
                }
                else if (distDroneToEnemy <= ceData.npcLD && distDroneToEnemy <= range2 && ceData.stopS == 0) {
            ...
            if (alignedTowardsEnemy1 > 0.75 && distDroneToEnemy <= ceData.npcLD && distDroneToEnemy <= range2 && ceData.stopS == 0 || alignedTowardsEnemy1 > 0.75 && distDroneToEnemy < ceData.npcLD && distDroneToEnemy <= range2 && ceData.tec2C >= 0 ||
                distDroneToEnemy <= ceData.npcLD && distDroneToEnemy <= range2)
            {
