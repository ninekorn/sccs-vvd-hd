/*
 ...
 Desc: flies around in search of asteroids
 ============================================================
 */
...
function OnAIInited()
{
    ...
    npc.SetBehavior(SHIP_ID, "run_away_on_hp", 70.0);
}
...
function Decision(args)
{
    ...
    {
        var asteroids = storage.Get("npc_" + SHIP_ID, "asteroids");
        var currentIndex = storage.Get("npc_" + SHIP_ID, "currentIndex");

        if (currentIndex == null)
        {
            currentIndex = 0;
            storage.Set("npc_" + SHIP_ID, "currentIndex", currentIndex);
        }

        //find next closest asteroid in the system
        if (asteroids == null)
        {
            asteroids = npc.GetSystemAsteroidsByDistanceToNPC(SHIP_ID, 10);
            storage.Set("npc_" + SHIP_ID, "asteroids", asteroids);
            currentIndex = 0;
        }

        var currentAsteroid = asteroids[currentIndex]; //store in global var
        currentIndex++;
        if (currentIndex >= asteroids.length)
        {
            currentIndex = 0;
        }
        storage.Set("npc_" + SHIP_ID, "currentIndex", currentIndex);
        ...
        //find some asteroid
        npc.AddObjective(SHIP_ID, "go_to_asteroid", { id: currentAsteroid });
        npc.AddObjective(SHIP_ID, "mine_for_time", { time: 4, id: currentAsteroid });
        npc.AddObjective(SHIP_ID, "mine_for_time", { time: 4, id: currentAsteroid });
        npc.AddObjective(SHIP_ID, "mine_for_time", { time: 4, id: currentAsteroid });
        npc.AddObjective(SHIP_ID, "mine_for_time", { time: 4, id: currentAsteroid });
    }
...
function ProcessObjectives(args)
{
    ...
    if (currentObjective.name == "go_to_asteroid")
    {
        npc.FollowRouteToObject(SHIP_ID, currentObjective.id);

        var dist = npc.GetDistanceToObj(SHIP_ID, currentObjective.id);
        if (dist < 11)
        {
    ...
    else if (currentObjective.name == "mine_for_time")
    {
        var activated = storage.Get("npc_" + SHIP_ID, "activated");
        if (activated == null || !activated)
        {
            npc.StopEvasion(SHIP_ID);
            npc.InstantStop(SHIP_ID);
            npc.DeviceActivateOnObject(SHIP_ID, "device_mining_fake", currentObjective.id);
            activated = true;
            storage.Set("npc_" + SHIP_ID, "activated", activated);
        }

        if (npc.GetCurrentObjectiveTimer(SHIP_ID) > currentObjective.time)
        {
            npc.StartEvasion(SHIP_ID);
            npc.DeviceDeactivate(SHIP_ID, "device_mining_fake");
            npc.NextObjective(SHIP_ID);
            activated = false;
            storage.Set("npc_" + SHIP_ID, "activated", activated);
        }
