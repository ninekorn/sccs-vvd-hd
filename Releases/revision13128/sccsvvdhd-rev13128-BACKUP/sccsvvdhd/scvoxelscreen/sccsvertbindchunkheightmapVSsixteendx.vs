...
static int tinyChunkWidth = 16; // 4 // 8
static int tinyChunkHeight = 16; // 4 // 5
static int tinyChunkDepth = 1; // 4 // 8 // 1
static const int maxfloatbytemaparraylength = 16; //4 // 8 
static const int maxfloatbytemaparraylengthfull = 17; //5 // 9
...
{ 
	...
	somecountermul = currentIndex / 4;

	int someotherindexx = currentIndex / 64;
	...
	if (someotherindexx == 0)
	{
		//1.0f
		//values are between 100.0f and 0.0f;
		currentMapData = ((round(currentMapData) * 0.001f) - round(currentMapData * 0.001f)) * 100.0f; 
		//if 10045.45451f
		//100.45f - ( 100.0f) = 0.45f
	}
	else if (someotherindexx == 1)
	{
		//0.001f
		//values are between 0.001f and 0.0f;
		currentMapData = round(((currentMapData) - round(currentMapData)) * 100.0f); 
		//if 10045.45451f
		//10045.45451f - 10045.0f
	}
	else if (someotherindexx == 2)
	{
		//0.00001f
		//values are between 0.00001f and 0.001f;
		currentMapData = round(((((currentMapData) - round(currentMapData)) * 100.0f) - round(((currentMapData) - round(currentMapData)) * 100.0f)) * 100.0f); 
		//if 10045.45451f
		//10045.45451f - 10045 = 0.45451f * 100 = 45.451f - 45.0f = 0.451f
	}
	else if (someotherindexx == 3)
	{
		//100f
		//values are between 10000f and 100f;
		currentMapData = round(currentMapData * 0.001f) - 100.0f;
	}
	...
	//input.color = float4(input.color.x + (input.position.x*0.1),input.color.y+ (input.position.y*0.1),input.color.z+ (input.position.z*0.1),input.color.w);
   	...
   	//float3 otherdir = mod_input_vertex_pos.xyz; //float3(0,0,-1);//
	//otherdir = rotatevecxaxis(otherdir.x,otherdir.y,otherdir.z,-25); //45
	...
	*/
	...
	float somemul = currentMapData * 0.01f;
	...
	if(facetype == 0) //frontface
	{
		if(input.color.z - floor(input.color.z) == 0.1f)
		{
			float3 otherdir = ovrpos.xyz - MOVINGPOINT.xyz *-1;  //ovrdirf.xyz * -1;//ovrpos.xyz - MOVINGPOINT.xyz; 
			//otherdir.x *=-1;
			//otherdir.y *=-1;
			float somelength = length(otherdir);
			otherdir /= somelength;
