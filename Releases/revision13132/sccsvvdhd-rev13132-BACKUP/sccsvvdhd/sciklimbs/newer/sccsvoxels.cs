...
namespace sccsr15forms
{
    ...
    public class sccsvoxels
    {
        ...
        private List<sc_voxel.DVertex> listOfVerts = new List<sc_voxel.DVertex>();
        ...
        //FUNCTION INSPIRED AND REFERENCED FROM CRAIG PERKO'S FIRST MINECRAFT TUTORIAL ON YOUTUBE AND HEAVILY MODIFIED BY STEVE CHASSÉ AKA NINEKORN/NINE/9
        //FUNCTION INSPIRED AND REFERENCED FROM CRAIG PERKO'S FIRST MINECRAFT TUTORIAL ON YOUTUBE AND HEAVILY MODIFIED BY STEVE CHASSÉ AKA NINEKORN/NINE/9
        public void startBuildingArray(Vector3 currentPosition, out sc_voxel.DVertex[] vertexArray, out int[] triangleArray, out int[] mapper, float _planeSize, int minx, int miny, int minz, float diagmaxx, float diagmaxy, float diagmaxz, float diagminx, float diagminy, float diagminz, int _ChunkWidth_L, int _ChunkWidth_R, int _ChunkHeight_L, int _ChunkHeight_R, int _ChunkDepth_L, int _ChunkDepth_R, float maxDistance, float vert_offset_x, float vert_offset_y, float vert_offset_z, int voxel_type, int voxelbodyparttype, int somechunkpriminstanceikarmvoxelindex) //, out int vertexNum, out int indicesNum //Vector3 currentPosition, out Vector3[] vertexArray, out int[] indicesArray, 
        {
            float maxx = 2;
            float maxy = 8;
            float maxz = 2;
        ...
        private void createTopFace(Vector3 start, Vector3 offset1, Vector3 offset2)
        {
            ...
            listOfVerts.Add(new sc_voxel.DVertex()
            {
                position = start,
