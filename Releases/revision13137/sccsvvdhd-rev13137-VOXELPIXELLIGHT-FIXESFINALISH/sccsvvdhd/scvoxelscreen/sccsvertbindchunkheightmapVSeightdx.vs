...
{ 
	...
	facetype = int(round(input.color.w));

	//int isfrontfacelinkedvertex = int(round(input.color.z - round(input.color.z)));
	int isfrontfacelinkedvertex = int(trunc(int((input.color.z - round(input.color.z)) * 10.0f)));
	...
	/*
	forwardDir = float3(0,0,1);//float3(input.instanceRadRotFORWARD.x, input.instanceRadRotFORWARD.y, input.instanceRadRotFORWARD.z);
	rightDir = float3(1,0,0);//float3(input.instanceRadRotRIGHT.x, input.instanceRadRotRIGHT.y, input.instanceRadRotRIGHT.z); 
	upDir = float3(0,1,0);//float3(input.instanceRadRotUP.x, input.instanceRadRotUP.y, input.instanceRadRotUP.z);
	*/

	/*
	forwardDir = input.instanceRadRotFORWARD.xyz;
	rightDir = input.instanceRadRotRIGHT.xyz;
	upDir = input.instanceRadRotUP.xyz;*/
	...
	*/
	...
	int instancesw = input.paddingvert0;//int(input.paddingvert0); // 256 // 240 // 320 // 480 // 128 //120
	int instancesh = input.paddingvert1;//int(input.paddingvert1); // 128 // 135 // 180 // 270 // 72 //68
	int instancesd = 1;//int(input.paddingvert2);

	if(int(round(theovrposmod.w)) == 0)
	{
		float3 postest = MOVINGPOINT;//input.position.xyz * 0.5f;//MOVINGPOINT; //input.position.xyz + input.instancePosition.xyz;
	...
	else if(int(round(theovrposmod.w)) == 1)
	{
		...
		float3 postest = MOVINGPOINT ; //input.position.xyz + input.instancePosition.xyz;
		
		//postest.x -= (instancesw * 8 * 0.5f);
		//postest.y -= (instancesh * 8 * 0.5f);
		...
		//float3 theinstancepos = input.instancePosition.xyz;

		//float3 dirinstancetoovrpos = theinstancepos - postest;
		//float thedirlength = length(dirinstancetoovrpos);
		//dirinstancetoovrpos /= thedirlength;
		...
		//somecamerapos = postest;//ovrpos.xyz;
		//somecamerapos.y *= -1;
		//somecamerapos.x *= -1;
	...
	//int vvdtype = int(round(lightextras.y));
	int vvdtype = int((lightextras.w - floor(lightextras.w)) * 10);
	...
	if(vvdtype == 3)
	{
		if(projectionsetting == 0)
		{
			...
			{
				if(int((input.color.z - round(input.color.z)) * 10 == 1))
				{
	...
	output.instanceRadRotFORWARD = input.position; //input.instanceRadRotFORWARD.x;
	output.instanceRadRotRIGHT = input.instanceRadRotFORWARD; 
