...
namespace sccs
{
    public class sccsikvoxellimbs : IDisposable
    {
        ...
        public scmessageobjectjitter[][] createikbody(scmessageobjectjitter[][] _sc_jitter_tasks, int tempMultiInstancePhysicsTotal, Vector3 ikarmpivotinitposition, sccsikvoxellimbs parentobject_, Matrix worldmatofobj_, int human_inst_rig_x, int human_inst_rig_y, int human_inst_rig_z, int grabtargetitem_, Matrix finalrotationmatrix, float ikvoxelrigbodysize)
        {
            ...
            diagmaxx *= sizemultiplier;
            diagmaxy *= sizemultiplier;
            diagmaxz *= sizemultiplier;

            diagminx *= sizemultiplier;
            diagminy *= sizemultiplier;
            diagminz *= sizemultiplier;

            chunkwidthl *= sizemultiplier;
            chunkwidthr *= sizemultiplier;

            chunkheightl *= sizemultiplier;
            chunkheightr *= sizemultiplier;

            chunkdepthl *= sizemultiplier;
            chunkdepthr *= sizemultiplier;
        ...
        public scmessageobjectjitter[][] setikbodytargetnlimbspositionsNrotations(scmessageobjectjitter[][] _sc_jitter_tasks, Matrix viewMatrix, Matrix projectionMatrix, Vector3 OFFSETPOS, Matrix originRot, Matrix rotatingMatrix, Matrix hmdrotMatrix, Matrix hmd_matrix, Matrix rotatingMatrixForPelvis, Matrix _rightTouchMatrix, Matrix _leftTouchMatrix, Matrix oriProjectionMatrix, sc_voxel mainarmparentmeshobject, Vector3 lightpos_, Vector3 dirLight_, Matrix finalRotationMatrix_, sc_voxel mainarmparentmeshobjectmain, Matrix hmd_matrix_current, Matrix extramatrix, Matrix hmdmatrixcurrentforpelvis) //Posef handPoseRight, Posef handPoseLeft, 
        {
            ...
            for (int _iterator = 0; _iterator < _human_inst_rig_x * _human_inst_rig_y * _human_inst_rig_z; _iterator++) // _player_torso[0][0]._arrayOfInstances.Length
            {
                ...
                */
                ...
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                var lengthoftorso = _player_torso[0][0]._total_torso_height;
                float diffx0 = originpositionikvoxelbody.X - MOVINGPOINTER.X;
                float diffy0 = originpositionikvoxelbody.Y - MOVINGPOINTER.Y;
                float diffz0 = originpositionikvoxelbody.Z - MOVINGPOINTER.Z;
                Vector3 dirpointtotargetpoint0 = originpositionikvoxelbody - MOVINGPOINTER;
                float lengthof1 = dirpointtotargetpoint0.Length();
                dirpointtotargetpoint0.Normalize();
                MOVINGPOINTER = MOVINGPOINTER + (dirpointtotargetpoint0 * lengthof1);
                MOVINGPOINTER = MOVINGPOINTER + (direction_feet_up * (lengthoftorso * 0.5f * 1.25f * 4.15f));
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                ...
                diffx0 = originpositionikvoxelbody.X - MOVINGPOINTER.X;
                diffy0 = originpositionikvoxelbody.Y - MOVINGPOINTER.Y;
                diffz0 = originpositionikvoxelbody.Z - MOVINGPOINTER.Z;
                dirpointtotargetpoint0 = originpositionikvoxelbody - MOVINGPOINTER;
                lengthof1 = dirpointtotargetpoint0.Length();
                dirpointtotargetpoint0.Normalize();
                MOVINGPOINTER = somepivotheadoffset + (dirpointtotargetpoint0 * lengthof1);
                MOVINGPOINTER = MOVINGPOINTER + (direction_feet_up * (lengthoftorso * 0.5f * 1.25f * 4.15f));
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                ...
                MOVINGPOINTER = MOVINGPOINTER + (direction_head_up * (_player_head[0][0]._total_torso_depth * 1.0f));
                //MOVINGPOINTER = MOVINGPOINTER + (-current_rotation_of_torso_pivot_up * (_player_head[0][0]._total_torso_depth * 2.0f));
                ...
                */
                ...
                /*float diffx = originpositionikvoxelbody.X - MOVINGPOINTER.X;
                float diffy = originpositionikvoxelbody.Y - MOVINGPOINTER.Y;
                float diffz = originpositionikvoxelbody.Z - MOVINGPOINTER.Z;

                Vector3 dirpointtotargetpoint = originpositionikvoxelbody - MOVINGPOINTER;

                float lengthof0 = dirpointtotargetpoint.Length();

                dirpointtotargetpoint.Normalize();
                MOVINGPOINTER = MOVINGPOINTER + (dirpointtotargetpoint * lengthof0);
                MOVINGPOINTER = MOVINGPOINTER + (direction_feet_up * (lengthoftorso* 0.5f * 1.25f * 2.0f));*/
        ...
        public scmessageobjectjitter[][] ikbodyrender(scmessageobjectjitter[][] _sc_jitter_tasks, Matrix viewMatrix, Matrix projectionMatrix, Vector3 OFFSETPOS, Matrix originRot, Matrix rotatingMatrix, Matrix hmdrotMatrix, Matrix hmd_matrix, Matrix rotatingMatrixForPelvis, Matrix _rightTouchMatrix, Matrix _leftTouchMatrix, Matrix oriProjectionMatrix) // Posef handPoseRight, Posef handPoseLeft,
        {
            ...
            scupdate._shaderManager._rend_torso(scdirectx.D3D.Device.ImmediateContext, _player_head[0][0].IndexCount, _player_head[0][0].InstanceCount, _player_head[0][0]._POSITION, viewMatrix, projectionMatrix, null, _SC_modL_torso_BUFFER, _player_head[0][0]);
        ...
        public scmessageobjectjitter[][] setiktargetnlimbspositionsNrotations(scmessageobjectjitter[][] _sc_jitter_tasks, Matrix viewMatrix, Matrix projectionMatrix, Vector3 OFFSETPOS, Matrix originRot, Matrix rotatingMatrix, Matrix hmdrotMatrix, Matrix hmd_matrix, Matrix rotatingMatrixForPelvis, Matrix _rightTouchMatrix, Matrix _leftTouchMatrix, Matrix oriProjectionMatrix, sc_voxel mainarmparentmeshobject, Vector3 lightpos_, Vector3 dirLight_, Matrix finalRotationMatrix_, sc_voxel mainarmparentmeshobjectmain, sccsikvoxellimbs ikvoxelbody, int somechunkpriminstanceikarmvoxelindex, Matrix hmd_matrix_current, Matrix extramatrix, Vector3[] directionvectoroffsets_, Vector3 targetfootleft, Vector3 targetfootright, int typeoflimb) // Posef handPoseRight, Posef handPoseLeft,
        {
            ...
            for (int _iterator = 0; _iterator < _human_inst_rig_x * _human_inst_rig_y * _human_inst_rig_z; _iterator++) // //_player_rght_hnd[0][0]._arrayOfInstances.Length
            {
                ...
                if (somechunkpriminstanceikarmvoxelindex == 0)
                {
                    ...
                    //var inithandmatrix = _player_rght_hnd[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION; //_leftTouchMatrix;
                    ...
                    var shoulderMatrix = ikvoxelbody._player_torso[0][0]._arrayOfInstances[_iterator].current_pos;
                    // var shoulderMatrix = _player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos;
                    ...
                    //var torsocurrentpos = new Vector3(ikvoxelbody._player_torso[0][0]._arrayOfInstances[_iterator].current_pos.M41, ikvoxelbody._player_torso[0][0]._arrayOfInstances[_iterator].current_pos.M42, ikvoxelbody._player_torso[0][0]._arrayOfInstances[_iterator].current_pos.M43);
                    var torsocurrentpos = new Vector3(_player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos.M41, _player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos.M42, _player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos.M43);

                    var thetorsorotation = Matrix.LookAtRH(torsocurrentpos, torsocurrentpos + dirtorsof, dirtorsou);
                    //thetorsorotation.Invert();

                    var lowerarmmatrix = _player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos;
                    ...
                    if (_iterator == 1)
                    {
                        ...
                        /*var shoulderMatrix0 = _player_rght_shldr[0][0]._arrayOfInstances[_iterator].current_pos;
                        // var shoulderMatrix = _player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos;

                        Quaternion.RotationMatrix(ref shoulderMatrix0, out otherQuat);
                        var dirtorsof0 = sc_maths._getDirection(Vector3.ForwardRH, otherQuat);
                        var dirtorsor0 = sc_maths._getDirection(Vector3.Right, otherQuat);
                        var dirtorsou0 = sc_maths._getDirection(Vector3.Up, otherQuat);

                        var grabcurrentpos = new Vector3(_player_r_hand_grab[0][0]._arrayOfInstances[_iterator].current_pos.M41, _player_r_hand_grab[0][0]._arrayOfInstances[_iterator].current_pos.M42, _player_r_hand_grab[0][0]._arrayOfInstances[_iterator].current_pos.M43);

                        Vector3 dirgrabtotarget = targetfootright - grabcurrentpos;
                        dirgrabtotarget.Normalize();

                        //var torsocurrentpos = new Vector3(ikvoxelbody._player_torso[0][0]._arrayOfInstances[_iterator].current_pos.M41, ikvoxelbody._player_torso[0][0]._arrayOfInstances[_iterator].current_pos.M42, ikvoxelbody._player_torso[0][0]._arrayOfInstances[_iterator].current_pos.M43);
                        var torsocurrentpos0 = new Vector3(_player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos.M41, _player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos.M42, _player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos.M43);

                        var thetorsorotation0 = Matrix.LookAtRH(grabcurrentpos, grabcurrentpos + dirtorsof0 , dirtorsou0);
                        */
                        matrixerer = thetorsorotation;//
                    }
                    else
                    ...
                    //matrixerer = thetorsorotation;//matrixerer.M41 = MOVINGPOINTER.X;
                ...
                else if (somechunkpriminstanceikarmvoxelindex == 3)
                {
                   ...
                   // var shoulderMatrix = _player_rght_lower_arm[0][0]._arrayOfInstances[_iterator].current_pos;
                    ...
                    //matrixerer = thetorsorotation;// inithandmatrix * finalRotationMatrix;// * somematrix;
                ...
                if (somechunkpriminstanceikarmvoxelindex == 0)
                {
                    ...
                    var matrixofpelvistorso = finalRotationMatrix;
                    Quaternion.RotationMatrix(ref matrixofpelvistorso, out otherQuat);
                    var dirtorsof = sc_maths._getDirection(Vector3.ForwardRH, otherQuat);
                    var dirtorsor = sc_maths._getDirection(Vector3.Right, otherQuat);
                    var dirtorsou = sc_maths._getDirection(Vector3.Up, otherQuat);
                    ...
                    if (_iterator == 0)
                    {
                        /*Vector3 theshoulderposition = new Vector3(_player_rght_shldr[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION.M41, _player_rght_shldr[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION.M42, _player_rght_shldr[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION.M43);
                        Vector3 dirtotargetobject = targetfootright - theshoulderposition;
                        dirtotargetobject.Normalize();

                        //MOVINGPOINTER = targetfootright;
                        MOVINGPOINTER += (dirtotargetobject * totalArmLengthRight0 * multiplierdist);*/

                        Vector3 theshoulderposition = new Vector3(_player_rght_shldr[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION.M41, _player_rght_shldr[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION.M42, _player_rght_shldr[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION.M43);
                        Vector3 dirtotargetobject = targetfootright - theshoulderposition;
                        dirtotargetobject.Normalize();

                        Vector3 theshoulderpositioncurrent = new Vector3(_player_rght_shldr[0][0]._arrayOfInstances[_iterator].current_pos.M41, _player_rght_shldr[0][0]._arrayOfInstances[_iterator].current_pos.M42, _player_rght_shldr[0][0]._arrayOfInstances[_iterator].current_pos.M43);

                        MOVINGPOINTER = MOVINGPOINTER + (dirtotargetobject * totalArmLengthRight0 * multiplierdist);
                    }
                    else
                    {
                        //MOVINGPOINTER += OFFSETPOS;

                        Vector3 originpositionikvoxelbody = new Vector3(_player_rght_shldr[0][0]._arrayOfInstances[_iterator].current_pos.M41, _player_rght_shldr[0][0]._arrayOfInstances[_iterator].current_pos.M42, _player_rght_shldr[0][0]._arrayOfInstances[_iterator].current_pos.M43);
                        /*if (_iterator == 0)
                        {
                            originpositionikvoxelbody = new Vector3(scgraphicssec.currentscgraphicssec.worldmatrixinstancesvoxelsforcalculationsonly[0][0][18].M41,
                                                            scgraphicssec.currentscgraphicssec.worldmatrixinstancesvoxelsforcalculationsonly[0][0][18].M42,
                        ...
                        dirpointtotargetpoint0.Normalize();
                        MOVINGPOINTER = MOVINGPOINTER + (dirpointtotargetpoint0 * lengthof1);
                        MOVINGPOINTER = MOVINGPOINTER + (-direction_head_up * (totalArmLengthRight * 0.5f * 1.25f * 4.15f));
                        //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                        //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                        //MOVING THE VOXEL HUMAN IK RIG ON TOP OF THE VOXEL CORNER CAMERA BOUNDS OBJECTS
                    }
                    ...
                    matrixerer = _player_rght_shldr[0][0]._arrayOfInstances[_iterator].current_pos;// Matrix.Identity;// * somematrix;
                    matrixerer.M41 = MOVINGPOINTER.X;
                    matrixerer.M42 = MOVINGPOINTER.Y;
                    matrixerer.M43 = MOVINGPOINTER.Z;
                    matrixerer.M44 = 1;
            ...
            for (int _iterator = 0; _iterator < _human_inst_rig_x * _human_inst_rig_y * _human_inst_rig_z; _iterator++) // //_player_rght_hnd[0][0]._arrayOfInstances.Length
            {
                ...
                */
                ...
                if (somechunkpriminstanceikarmvoxelindex == 0)
                {
                    ...
                    if (_iterator == 0)
                    {
                        ...
                        if (Program._useOculusRift == 1)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    somevec0 = (somevec0) + (-dirshoulderright0 * ((lengthOfHandheight * 0.5f * 1.0923f)));
                                }
                        ...
                        else if (Program._useOculusRift == 0)
                        {
                            ...
                            if (Program.updatescript.typeofsensortouchR != 9999999)
                            {
                                if (somechunkpriminstanceikfingervoxelindex != 3)
                                {
                                    ...
                                    if (Program.updatescript.handTriggerRight[1] > 0.00000001f)
                                    {
                                        somevec0 = (somevec0) + (dirshoulderright0 * ((lengthOfHandheight * Program.updatescript.handTriggerRight[1] * 4.0f)));
                                    }
                                    else
                                    {
                                        somevec0 = (somevec0) + (dirshoulderright0 * ((lengthOfHandheight * 0.05f * 1.0923f)));
                                    ...
                                    if (Program.updatescript.handTriggerRight[1] > 0.00000001f) ///when over 0.52f
                                    {
                                        somevec0 = (somevec0) + (dirshoulderforward0 * ((lengthOfHandheight * (1 - Program.updatescript.handTriggerRight[1]) * 4.0f)));
                                    }
                                    else
                                    {
                                        somevec0 = (somevec0) + (dirshoulderforward0 * ((lengthOfHandheight * 0.5f * 1.0923f)));
                    ...
                    matrixerer = _player_rght_hnd[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION * finalRotationMatrix;
                ...
                else if (somechunkpriminstanceikarmvoxelindex == 3)
                {
                    ...
                    //matrixerer = _player_rght_hnd[0][0]._arrayOfInstances[_iterator]._ORIGINPOSITION;
