...
namespace sccs
{
    public class SC_VR_Chunk
    {
        ...
        public SC_VR_Chunk(SharpDX.Direct3D11.Device device, float xi, float yi, float zi, Vector4 color, int width, int height, int depth, Vector3 pos,int mapWidth_, int mapHeight_, int mapDepth_, int tinyChunkWidth_, int tinyChunkHeight_,int tinyChunkDepth_, int mapObjectInstanceWidth_, int mapObjectInstanceHeight_,int mapObjectInstanceDepth_, float planesize_) //,DInstanceType[] _instances
        {
            ...
            var vsFileNameByteArray = sccsr14sc.Properties.Resources.textureTrigVS;
            var psFileNameByteArray = sccsr14sc.Properties.Resources.textureTrigPS;
            var gsFileNameByteArray = sccsr14sc.Properties.Resources.geometryshader;
            ...
            for (int x = 0; x < mapWidth; x++)
            {
                for (int y = 0; y < mapHeight; y++)
                {
                    for (int z = 0; z < mapDepth; z++)
                    {
                        ...
                        int[] tester = newChunker.startBuildingArray(position, out vertexArray0, out indicesArray0, out themap, out arrayOfD, out normals, out texturesCoordinates, planeSize);
            ...
            for (int i = 0;i < mapWidth * mapHeight * mapDepth;i++ )
            {
               var matrixBufferDescriptionVertex = new BufferDescription()
                {
                    Usage = ResourceUsage.Dynamic,
                    SizeInBytes = Marshal.SizeOf(typeof(SC_VR_Chunk.DVertex)),
                    BindFlags = BindFlags.VertexBuffer,
                    CpuAccessFlags = CpuAccessFlags.Write,
                    OptionFlags = ResourceOptionFlags.None,
                    StructureByteStride = 0
                };
                vertBuffers[i] = new SharpDX.Direct3D11.Buffer(device, matrixBufferDescriptionVertex);
                ...
                normalBuffers[i] = new SharpDX.Direct3D11.Buffer(device, matrixBufferDescriptionVertex);
                ...
                texBuffers[i] = new SharpDX.Direct3D11.Buffer(device, matrixBufferDescriptionVertex);

                indexBuffers[i] = SharpDX.Direct3D11.Buffer.Create(device, BindFlags.IndexBuffer, arrayOfInstanceIndices[i]);
