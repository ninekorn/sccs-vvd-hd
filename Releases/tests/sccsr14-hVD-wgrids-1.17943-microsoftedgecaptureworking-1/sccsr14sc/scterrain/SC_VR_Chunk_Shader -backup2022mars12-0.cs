                    ...
                    if (chunkdat.arrayOfInstanceVertex[xx + mapWidth * (yy + mapHeight * zz)].Length > 0)
                    {
                        if (startOnce == 1)
                        {
                            ...
                            */
                            ...
                            //chunkdat.vertBuffers[xx + mapWidth * (yy + mapHeight * zz)] = vertBuffers[xx + mapWidth * (yy + mapHeight * zz)];
                           ...
                           //chunkdat.texBuffers[xx + mapWidth * (yy + mapHeight * zz)] = texBuffers[xx + mapWidth * (yy + mapHeight * zz)];
                            ...
                            //chunkdat.lightBuffers[xx + mapWidth * (yy + mapHeight * zz)] = lightBuffers[xx + mapWidth * (yy + mapHeight * zz)];
                            ...
                            //chunkdat.constantBuffers[xx + mapWidth * (yy + mapHeight * zz)] = constantBuffers[xx + mapWidth * (yy + mapHeight * zz)];
                        ...
                        device.ImmediateContext.MapSubresource(instanceBuffers[xx + mapWidth * (yy + mapHeight * zz)], MapMode.WriteDiscard, SharpDX.Direct3D11.MapFlags.None, out mappedResource);
                        mappedResource.WriteRange(chunkdat.arrayOfInstancePos, 0, chunkdat.arrayOfInstancePos.Length);
                        device.ImmediateContext.UnmapSubresource(instanceBuffers[xx + mapWidth * (yy + mapHeight * zz)], 0);
                        mappedResource.Dispose();
                        ...
                        device.ImmediateContext.InputAssembler.SetVertexBuffers(1, new[]
                     {
                        ...
                        device.ImmediateContext.InputAssembler.SetVertexBuffers(0, new[]
                        {
                            new VertexBufferBinding(dVertBuffers[xx+mapWidth*(yy+mapHeight*zz)],Marshal.SizeOf(typeof(SC_VR_Chunk.DVertex)), 0),
                        });

                        device.ImmediateContext.InputAssembler.SetIndexBuffer(indexBuffers[xx + mapWidth * (yy + mapHeight * zz)], SharpDX.DXGI.Format.R32_UInt, 0);


                        device.ImmediateContext.InputAssembler.SetVertexBuffers(1, new[]
                        {
                            new VertexBufferBinding(instanceBuffers[xx + mapWidth * (yy + mapHeight * zz)], Utilities.SizeOf<SC_VR_Chunk.DInstanceType>(),0), //chunkdat.instanceBuffers[xx+mapWidth*(yy+mapHeight*zz)]
                        });
