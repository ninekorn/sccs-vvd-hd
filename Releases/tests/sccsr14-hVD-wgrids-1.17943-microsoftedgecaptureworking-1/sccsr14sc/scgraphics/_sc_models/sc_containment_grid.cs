...
namespace SCCoreSystems.SC_Graphics
{
    public class sc_containment_grid : ITransform, IComponent
    {
        ...
        public bool Initialize(SCCoreSystems.sc_console.SC_console_directx D3D, int width, int height, float tileSize, int divX, int divY, float _sizeX, float _sizeY, float _sizeZ, Vector4 color, int instX, int instY, int instZ, IntPtr windowsHandle, Matrix matroxer, int isTerrain, float offsetPosX, float offsetPosY, float offsetPosZ, World the_world, SCCoreSystems.sc_console.SC_console_directx.BodyTag tag, bool is_static, int addtoworld, float mass, float vertoffsetx, float vertoffsety, float vertoffsetz, float offsetX, float offsetY, float offsetZ, bool faceBottom, bool faceTop, bool faceLeft, bool faceRight, bool faceFront, bool faceBack)
        {
            ...
            if (!InitializeBuffer(D3D, _sizeX, _sizeY, _sizeZ, tileSize, instX, instY, instZ, windowsHandle, matroxer, isTerrain, offsetPosX, offsetPosY, offsetPosZ, vertoffsetx, vertoffsety, vertoffsetz,  offsetX,  offsetY,  offsetZ,  faceBottom,  faceTop,  faceLeft,  faceRight,  faceFront,  faceBack))
                return false;
        ...
        private bool InitializeBuffer(SCCoreSystems.sc_console.SC_console_directx D3D, float _sizeX, float _sizeY, float _sizeZ, float tileSize, int instX, int instY, int instZ, IntPtr windowsHandle, Matrix matroxer, int type_of_cube, float offsetPosX, float offsetPosY, float offsetPosZ, float vertoffsetx, float vertoffsety, float vertoffsetz, float offsetX, float offsetY, float offsetZ, bool faceBottom, bool faceTop, bool faceLeft, bool faceRight, bool faceFront, bool faceBack)
        {
            try
            {
                int tessellation = 3;
                    ...
                    for (int j = -m_TerrainWidth; j < (m_TerrainWidth); j++)
                    {
                        for (int i = -m_TerrainHeight; i < (m_TerrainHeight); i++)
                        {
                            if (index < VertexCount)
                            {
                                ...
                                //Vertices = new[] { };                                 
                                //TOP
                                Vertices[index] = new DVertex()
                                {
                                    position = new Vector3(positionX, 0.001f, positionZ) * _tileSize,
                                    texture = new Vector2(0, 0),
                                    color = _color,
                                    normal = new Vector3(0, 1, 1),
                                };
