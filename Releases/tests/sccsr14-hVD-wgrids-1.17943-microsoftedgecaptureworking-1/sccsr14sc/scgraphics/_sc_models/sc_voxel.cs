...
namespace SCCoreSystems.SC_Graphics
{
    public class sc_voxel : ITransform, IComponent
    {
        ...
        int[] map;
        int[] triangles;
        private bool InitializeBuffer(SCCoreSystems.sc_console.SC_console_directx D3D, int width, int height, int depth, float sizex, float sizey, float sizez, float tileSize, int instX, int instY, int instZ, IntPtr windowsHandle, Matrix matroxer, int isTerrain, float offsetPosX, float offsetPosY, float offsetPosZ, SCCoreSystems.sc_console.SC_console_directx.BodyTag _tag, int minx, int miny, int minz, float diagmaxx, float diagmaxy, float diagmaxz, float diagminx, float diagminy, float diagminz, int ChunkWidth_L, int ChunkWidth_R, int ChunkHeight_L, int ChunkHeight_R, int ChunkDepth_L, int ChunkDepth_R, float planesize, Vector3 offset_pos, float maxDistance, float vertoffsetx, float vertoffsety, float vertoffsetz)
        {
            try
            {
                List<Vector3> verts;
                ...
                try
                {
                    ...
                    //same chunk for every godamn instances. and here is why i need my 1stversion MainWindow.
                    _chunk.startBuildingArray(Vector3.Zero,
                                           out _dvertexarray,
                                           out triangles,
                                           out map,
                                           planesize, minx, miny, minz, diagmaxx, diagmaxy, diagmaxz, diagminx, diagminy, diagminz, ChunkWidth_L, ChunkWidth_R, ChunkHeight_L, ChunkHeight_L, ChunkDepth_L, ChunkDepth_R, maxDistance, vertoffsetx, vertoffsety, vertoffsetz, voxel_type, voxelbodyparttype, somechunkikarmpriminstanceikarmvoxelindex);
