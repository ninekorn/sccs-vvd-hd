...
namespace Win32.Shared
{
    ...
    public partial class WindowPicker : Window, IDisposable
    {
        ...
        public void createwindowpickerandinit()
        {
            InitializeComponent();
            Loaded += OnLoaded;
        ...
        public void createwindowpicker()
        {
            //InitializeComponent();
            Loaded += OnLoadedAgain;


        }
        private void OnLoadedAgain(object sender, RoutedEventArgs e)
        {
            //FindWindows();
        }
        ...
        public IntPtr PickCaptureTarget(IntPtr hWnd, IntPtr capturedappintptr, string programname)
        {
            ...
            */
            ...
            /* if (capturablewindowlist)
             {

             }*/


            for (int i = 0; i < capturablewindowlist.Count;i++)
            {
                if (capturablewindowlist[i].Title.ToLower().Contains("void") && capturablewindowlist[i].Title.ToLower().Contains("expanse"))
                {
        ...
        //System.Windows.Controls.ItemCollection capturablewindowcollection;// = new System.Windows.Controls.ItemCollection();

        List<CapturableWindow> capturablewindowlist = new List<CapturableWindow>();
        ...
        public void FindWindows()
        {
            ...
            NativeMethodsother.EnumWindows((hWnd, lParam) =>
            {
                ...
                var capturablewindow = new CapturableWindow
                {
                    Handle = hWnd,
                    Name = $"{title} ({process.ProcessName}.exe)",
                    Title = $"{title}"
                ...
                capturablewindowlist.Add(capturablewindow);
