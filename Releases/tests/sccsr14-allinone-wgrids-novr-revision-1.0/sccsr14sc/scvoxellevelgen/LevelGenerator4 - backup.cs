...
namespace sccs
{
    public class LevelGenerator4
    {
        ...
        void checkAllSides(Vector3 currentTilePos, KeyValuePair<Vector3, int> currentTuile)
        {
            ...
            for (var x = 1; x <= 1; x ++)
            {
                for (var z = 1; z <= 1; z ++)
                {
                    ...
                    if (checkX == currentTilePos.X && checkY == currentTilePos.Z + (otherchunksize) ||
                        checkX == currentTilePos.X && checkY == currentTilePos.Z - (otherchunksize) ||
                        checkX == currentTilePos.X + (otherchunksize) && checkY == currentTilePos.Z ||
                        checkX == currentTilePos.X - (otherchunksize) && checkY == currentTilePos.Z ||

                        checkX == currentTilePos.X + (otherchunksize) && checkY == currentTilePos.Z + (otherchunksize) ||
                        checkX == currentTilePos.X - (otherchunksize) && checkY == currentTilePos.Z + (otherchunksize) ||
                        checkX == currentTilePos.X + (otherchunksize) && checkY == currentTilePos.Z - (otherchunksize) ||
                        checkX == currentTilePos.X - (otherchunksize) && checkY == currentTilePos.Z - (otherchunksize))
                    {
        ...
        void buildWalls(Vector3 currentTile)
        {
            ...
            /////////////////////////////////////LEFTWALL/////////////////////////////////////////
            bool leftTilly0 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly0 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            bool frontWally0 = findWalls(currentTile.X, currentTile.Z + otherchunksize);
            bool backWally0 = findWalls(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally0 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally0 = findWalls(currentTile.X + otherchunksize, currentTile.Z);
            ...
            bool leftTilly1 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly1 = findTiles(currentTile.X + otherchunksize, currentTile.Z);
            bool frontWally1 = findWalls(currentTile.X, currentTile.Z + otherchunksize);
            bool backWally1 = findWalls(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally1 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally1 = findWalls(currentTile.X + otherchunksize, currentTile.Z);
            ...
            bool leftTilly2 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly2 = findWalls(currentTile.X + otherchunksize, currentTile.Z);
            bool frontTilly2 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool backTilly2 = findTiles(currentTile.X, currentTile.Z - otherchunksize);

            bool frontWally2 = findWalls(currentTile.X, currentTile.Z + otherchunksize);
            bool backWally2 = findWalls(currentTile.X, currentTile.Z - otherchunksize);
            ...
            bool leftTilly3 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly3 = findWalls(currentTile.X + otherchunksize, currentTile.Z);
            bool frontTilly3 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool backTilly3 = findTiles(currentTile.X, currentTile.Z - otherchunksize);

            bool frontWally3 = findWalls(currentTile.X, currentTile.Z + otherchunksize);
            bool backWally3 = findWalls(currentTile.X, currentTile.Z - otherchunksize);
            ...
            bool leftTilly4 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            //bool rightTilly4 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            bool frontTilly4 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool frontWally4 = findWalls(currentTile.X, currentTile.Z + otherchunksize);

            bool backWally4 = findWalls(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally4 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally4 = findWalls(currentTile.X + otherchunksize, currentTile.Z);
            ...
            //bool leftTilly5 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly5 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            bool frontTilly5 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool frontWally5 = findWalls(currentTile.X, currentTile.Z + otherchunksize);

            bool backWally5 = findWalls(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally5 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally5 = findWalls(currentTile.X + otherchunksize, currentTile.Z);
            ...
            bool leftTilly6 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            //bool rightTilly6 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            //bool frontTilly6 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool frontWally6 = findWalls(currentTile.X, currentTile.Z + otherchunksize);

            bool backWally6 = findWalls(currentTile.X, currentTile.Z - otherchunksize);
            bool backTilly6 = findTiles(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally6 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally6 = findWalls(currentTile.X + otherchunksize, currentTile.Z);
            ...
            //bool leftTilly7 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly7 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            //bool frontTilly7 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool frontWally7 = findWalls(currentTile.X, currentTile.Z + otherchunksize);

            bool backWally7 = findWalls(currentTile.X, currentTile.Z - otherchunksize);
            bool backTilly7 = findTiles(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally7 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally7 = findWalls(currentTile.X + otherchunksize, currentTile.Z);
            ...
            //bool leftTilly8 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly8 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            //bool frontTilly8 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool frontWally8 = findWalls(currentTile.X, currentTile.Z + otherchunksize);

            bool backWally8 = findWalls(currentTile.X, currentTile.Z - otherchunksize);
            bool backTilly8 = findTiles(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally8 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally8 = findWalls(currentTile.X + otherchunksize, currentTile.Z);

            bool leftFrontTilly8 = findTiles(currentTile.X - otherchunksize, currentTile.Z + otherchunksize);
            bool leftFrontWally8 = findWalls(currentTile.X - otherchunksize, currentTile.Z + otherchunksize);

            bool rightBackTilly8 = findTiles(currentTile.X + otherchunksize, currentTile.Z - otherchunksize);
            bool rightBackWally8 = findWalls(currentTile.X + otherchunksize, currentTile.Z - otherchunksize);
            ...
            bool leftWally9 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly9 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            //bool frontTilly9 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool frontWally9 = findWalls(currentTile.X, currentTile.Z + otherchunksize);

            bool backWally9 = findWalls(currentTile.X, currentTile.Z - otherchunksize);
            bool backTilly9 = findTiles(currentTile.X, currentTile.Z - otherchunksize);

            bool leftTilly9 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally9 = findWalls(currentTile.X + otherchunksize, currentTile.Z);

            bool leftBackTilly9 = findTiles(currentTile.X - otherchunksize, currentTile.Z - otherchunksize);
            bool leftBackWally9 = findWalls(currentTile.X - otherchunksize, currentTile.Z - otherchunksize);

            bool rightFrontWally9 = findWalls(currentTile.X + otherchunksize, currentTile.Z + otherchunksize);
            bool rightFrontTilly9 = findTiles(currentTile.X + otherchunksize, currentTile.Z + otherchunksize);
            ...
            bool leftTilly10 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly10 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            bool frontTilly10 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool frontWally10 = findWalls(currentTile.X, currentTile.Z + otherchunksize);

            bool backWally10 = findWalls(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally10 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally10 = findWalls(currentTile.X + otherchunksize, currentTile.Z);

            bool leftBackTilly10 = findTiles(currentTile.X - otherchunksize, currentTile.Z - otherchunksize);
            bool leftBackWally10 = findWalls(currentTile.X - otherchunksize, currentTile.Z - otherchunksize);

            bool rightFrontWally10 = findWalls(currentTile.X + otherchunksize, currentTile.Z + otherchunksize);
            bool rightFrontTilly10 = findTiles(currentTile.X + otherchunksize, currentTile.Z + otherchunksize);
            ...
            bool leftTilly11 = findTiles(currentTile.X - otherchunksize, currentTile.Z);
            bool rightTilly11 = findTiles(currentTile.X + otherchunksize, currentTile.Z);

            bool frontTilly11 = findTiles(currentTile.X, currentTile.Z + otherchunksize);
            bool frontWally11 = findWalls(currentTile.X, currentTile.Z + otherchunksize);

            bool backWally11 = findWalls(currentTile.X, currentTile.Z - otherchunksize);

            bool leftWally11 = findWalls(currentTile.X - otherchunksize, currentTile.Z);
            bool rightWally11 = findWalls(currentTile.X + otherchunksize, currentTile.Z);

            bool leftFrontTilly11 = findTiles(currentTile.X - otherchunksize, currentTile.Z + otherchunksize);
            bool leftFrontWally11 = findWalls(currentTile.X - otherchunksize, currentTile.Z + otherchunksize);

            bool rightBackTilly11 = findTiles(currentTile.X + otherchunksize, currentTile.Z - otherchunksize);
            bool rightBackWally11 = findWalls(currentTile.X + otherchunksize, currentTile.Z - otherchunksize);
        ...
        bool findWalls(float x, float z)
        {
            ...
            while (enumerator0.MoveNext())
            {
                ...
                if ((x < tuile.X) || (z < tuile.Z) || (x >= (tuile.X) + otherchunksize) || (z >= (tuile.Z) + otherchunksize))
                {
                    continue;
                }
                return true;
        ...
        /*bool findWalls(float x, float z)
        {
            ...
            while (enumerator0.MoveNext())
            {
                ...
                if ((x < tuile.x + blockSize) || (z < tuile.z + blockSize) || (x >= (tuile.x + blockSize) + otherchunksize ) || (z >= (tuile.z + blockSize) + otherchunksize ))
                {
                    continue;
                }
                return true;
