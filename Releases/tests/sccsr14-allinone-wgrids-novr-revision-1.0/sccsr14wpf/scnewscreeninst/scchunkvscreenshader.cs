...
namespace sccs
{
    public class scchunkvscreenshader // : System.Windows.Forms.Application
    {
        ...
        public scchunkvscreenshader(SharpDX.Direct3D11.Device _device, SharpDX.Direct3D11.Buffer _constantBuffer, InputLayout _Layout, VertexShader _VertexShader, PixelShader _PixelShader,GeometryShader _geoShader, SharpDX.Direct3D11.Buffer _instanceBuffer, SharpDX.Direct3D11.Buffer _ConstantLightBuffer)
        {
            ...
            mapWidth = scchunkvscreen.mapWidth;
            mapHeight = scchunkvscreen.mapHeight;
            mapDepth = scchunkvscreen.mapDepth;

            tinyChunkWidth = scchunkvscreen.tinyChunkWidth;
            tinyChunkHeight = scchunkvscreen.tinyChunkHeight;
            tinyChunkDepth = scchunkvscreen.tinyChunkDepth;

            mapObjectInstanceWidth = scchunkvscreen.mapObjectInstanceWidth;
            mapObjectInstanceHeight = scchunkvscreen.mapObjectInstanceHeight;
            mapObjectInstanceDepth = scchunkvscreen.mapObjectInstanceDepth;
        ...
        public void Renderer(sccs.scchunkvscreen.chunkData chunkdat) //async
        {
            try
            {
                //timeCalculator.Stop();
                ...
                */
                ...
                {
                    new VertexBufferBinding(instanceBuffer, Utilities.SizeOf<scchunkvscreen.DInstanceType>(),0), //chunkdat.instanceBuffers[xx+mapWidth*(yy+mapHeight*zz)]
                });
                ...
                for (int t = 0; t < total; t++)
                {
                    if (chunkdat.arrayOfInstanceVertex[xx + mapWidth * (yy + mapHeight * zz)].Length > 0)
                    {
                        if (startOnce == 1)
                        {
                            ...
                            {
                                Usage = ResourceUsage.Dynamic,
                                SizeInBytes = Marshal.SizeOf(typeof(scchunkvscreen.DVertex)) * chunkdat.dVertexData[xx + mapWidth * (yy + mapHeight * zz)].Length,
                                BindFlags = BindFlags.VertexBuffer,
                                CpuAccessFlags = CpuAccessFlags.Write,
                                OptionFlags = ResourceOptionFlags.None,
                                StructureByteStride = 0
                        ...
                        {
                            new VertexBufferBinding(chunkdat.dVertBuffers[xx+mapWidth*(yy+mapHeight*zz)],Marshal.SizeOf(typeof(scchunkvscreen.DVertex)), 0),
                        });
