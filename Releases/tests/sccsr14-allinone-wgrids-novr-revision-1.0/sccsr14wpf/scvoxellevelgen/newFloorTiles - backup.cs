...
public class newFloorTiles //: MonoBehaviour
{
   ...
   public void StartGenFaces(Vector3 positionofchunk)
    {

        chunkposition = positionofchunk;
        ...
        //chunks.Add(this);
        chunkz.Add(this, positionofchunk);
        chunks.Add(this);

        xChunkPos = positionofchunk.x;
        yChunkPos = positionofchunk.y;
        zChunkPos = positionofchunk.z;
    ...
    void RandomFillMap()
    {
        ...
        {
            float noiseX = Math.Abs((float)(x * planeSize + chunkposition.X + seed0) / 100);
            for (int y = 0; y < height; y++)
            {
                float noiseY = Math.Abs((float)(y * planeSize + chunkposition.Y + seed0) / 100);
                for (int z = 0; z < width; z++)
                {
                    float noiseZ = Math.Abs((float)(z * planeSize + chunkposition.Z + seed0) / 100);
        ...
        for (int j = 0; j < leftWall.Count; j++)
        {
            if (new Vector3(xChunkPos, yChunkPos, zChunkPos) == leftWall[j])
            {
                ...
                {
                    float noiseX = Math.Abs((float)(x * planeSize + chunkposition.X + seed0) / 100);
                    float noiseX2 = Math.Abs((float)(x * planeSize + chunkposition.X + seed0) / 25);
                    for (int y = 0; y < height; y++)
                    {
                        float noiseY = Math.Abs((float)(y * planeSize + chunkposition.Y + seed0) / 100);
                        float noiseY2 = Math.Abs((float)(y * planeSize + chunkposition.Y + seed0) / 25);
                        for (int z = 0; z < width; z++)
                        {
                            float noiseZ = Math.Abs((float)(z * planeSize + chunkposition.Z + seed0) / 100);
                            float noiseZ2 = Math.Abs((float)(z * planeSize + chunkposition.Z + seed0) / 25);
        ...
        for (int j = 0; j < LevelGenerator4.currentLevelGen.frontWall.Count; j++)
        {
            if (new Vector3(xChunkPos, yChunkPos, zChunkPos) == LevelGenerator4.currentLevelGen.frontWall[j])
            {
                ...
                {
                    float noiseX = Math.Abs((float)(x * planeSize + chunkposition.X + seed0) / 100);
                    float noiseX5 = Math.Abs((float)(x * planeSize + chunkposition.X + seed0) / 25);
                    for (int y = 0; y < height; y++)
                    {
                        float noiseY = Math.Abs((float)(y * planeSize + chunkposition.Y + seed0) / 100);
                        float noiseY5 = Math.Abs((float)(y * planeSize + chunkposition.Y + seed0) / 25);
                        for (int z = 0; z < width; z++)
                        {
                            float noiseZ = Math.Abs((float)(z * planeSize + chunkposition.Z + seed0) / 100);
                            float noiseZ5 = Math.Abs((float)(z * planeSize + chunkposition.Z + seed0) / 25);
    ...
    void drawBrick(int x, int y, int z, byte block)
    {
        ...
        if (x == 0)
        {
            ...
            {
                if (colliders[i].transform.position == new Vector3(chunkposition.X - chunkWidth, chunkposition.Y, chunkposition.Z))
                {
        ...
        if (z == chunkWidth - 1)
        {
            ...
            for (int i = 0; i < colliders.Length; i++)
            {
                if (colliders[i].transform.position == new Vector3(chunkposition.X, chunkposition.Y, chunkposition.Z + chunkWidth))
                {
                    //Instantiate(sphere, colliders[i].transform.position, Quaternion.identity);
                ...
                if (colliders[i].transform.position == new Vector3(chunkposition.X, chunkposition.Y, chunkposition.Z - chunkWidth))
                {
                    ...
                    if (isTransparent(x, y, z - 1))
                    {
                        if (colliders[i].GetComponent<newFloorTiles>().GetByte(x, y, z + chunkWidth - 1) != 1)
                        {
                            ...
                            offset2 = Vector3.Up * tileHeight;
                            DrawFrontFace(start + Vector3.Right + Vector3.BackwardRH, offset1, offset2, x, y, z);
        ...
        if (z == 0)
        {
            ...
            for (int i = 0; i < colliders.Length; i++)
            {
                if (colliders[i].transform.position == new Vector3(chunkposition.X, chunkposition.Y - chunkWidth, chunkposition.Z))
                {
                    //Instantiate(sphere, colliders[i].transform.position, Quaternion.identity);
                    if (isTransparent(x, y - 1, z))
                    {
                        ...
                        {
                            offset1 = Vector3.BackwardRH;//(new Vector3(1, 0, 0));
        ...
        if (y > 0 && y <= height - 1)
        {
            if (isTransparent(x, y - 1, z))
            {
                offset1 = Vector3.BackwardRHwardRH;//(new Vector3(1, 0, 0));
                offset2 = Vector3.Right;//(new Vector3(0, 0, 1));
                DrawBottomFace(start + Vector3.Down + Vector3.Up, offset1, offset2, x, y, z);
            }
        }*/
...
/*
 *    if (chunkposition.X <= -widthWorld && chunkposition.Z <= -widthWorld)
                {
                    float noiseX = Math.Abs((float)(x * planeSize + chunkposition.X + seed0) / 40);
                    float noiseY = Math.Abs((float)(y * planeSize + chunkposition.Y + seed0) / 40);
                    float noiseZ = Math.Abs((float)(z * planeSize + chunkposition.Z + seed0) / 40);
