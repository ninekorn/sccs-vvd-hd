...
namespace sccs
{
    public class sclevelgenchunkshader // : System.Windows.Forms.Application
    {
        ...
        public void Renderer(sccs.sclevelgenchunk.chunkData chunkdat) //async
        {
            try
            {
                //timeCalculator.Stop();
                ...
                for (int t = 0; t < total; t++)
                {
                    if (chunkdat.arrayOfInstanceVertex[xx + mapWidth * (yy + mapHeight * zz)].Length > 0)
                    {
                        if (startOnce == 1)
                        {
                            ...
                            {
                                Usage = ResourceUsage.Dynamic,
                                SizeInBytes = Marshal.SizeOf(typeof(sclevelgenchunk.DVertex)) * chunkdat.dVertexData[xx + mapWidth * (yy + mapHeight * zz)].Length,
                                BindFlags = BindFlags.VertexBuffer,
                                CpuAccessFlags = CpuAccessFlags.Write,
                                OptionFlags = ResourceOptionFlags.None,
                                StructureByteStride = 0
                        ...
                        {
                            new VertexBufferBinding(chunkdat.dVertBuffers[xx+mapWidth*(yy+mapHeight*zz)],Marshal.SizeOf(typeof(sclevelgenchunk.DVertex)), 0),
                        });
